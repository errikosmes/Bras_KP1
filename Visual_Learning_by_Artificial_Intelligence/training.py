...
class MyModel:
    def __init__(self, input_shape, output_len):
        ...
        self.model.add(Conv2D(64, kernel_size=5, activation='relu'))
    ...
    for name in objects_names:
        ...
        for file_name in list_dir:
            ...
            for x in range(0, len(objs)):
                ...
                cv2.imwrite("data_mask1/" + name + "/" + str(x) + "_" + file_name, img)
...
def training():
    ...
    *data_set, objects_names = load_dataset("data/")
    ...
    history = model.model.fit(
        datagen.flow(object_train, labels_train, batch_size=16), steps_per_epoch=int(len(object_train)/16),
        epochs=100
    )
